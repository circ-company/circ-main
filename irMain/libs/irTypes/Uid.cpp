...
Uid::Uid(const bool random) { generate(random); }
...
Uid Uid::generate(const bool random)
{
    if (random)
        generate4();
    else
...
Uid Uid::generate(const Version ver)
{
    ...
    case VerGTimeseqNode6:      result.generate(ver, NetworkMacAddress(true));      break;
    case VerRandom:
    case $nullVer:              result.nilify();                                    break;
    case $invalidVer:
    default:                    result.nullify();                                   break;
    };
    result.set(VarDce);
    result.set(ver);
    return result;
...
Uid Uid::generate(const Version ver, const NetworkMacAddress &mac, const QDateTime &dttm)
{
    Uid result;
    set(ver);
    GerogianHNanoTime tGHNT;
    tGHNT.generate(mac, dttm);
    insert(result, ver, tGHNT.hnano(), tGHNT.sequence(), mac);
    return result;
}

Uid Uid::generate4(const Uid override)
{
    Uid result; // null
    if ( ! override.isNil())
        result.qUnion().data128[0] = override.qUnion().data128[0];
    else
        randomize();
    set(VerCustom);
    return result;
